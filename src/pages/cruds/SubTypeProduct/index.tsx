...
import productService from '../../../util/subtipoproduto.json';
import { Input } from '@/components/Input.tsx'
import {Errors} from "@/components/Errors.tsx";
const TypeProductSchema = z.object({
  idsubtipoproduto: z.number().nullable(),
...
export function SubTypeProduct() {
  const defaultValues = {
    descricao: '',
    idsubtipoproduto: 0,
  ...
  function editProduct(rowData: TypeProductInputs) {
    ...
    setValue('idsubtipoproduto', rowData.idsubtipoproduto)
    setProductDialog(true);
  }
  ...
  function confirmDeleteSubTypeProdct(data: TypeProductInputs) {
    setProduct(data)
  ...
  function deleteProductDialogFooter() {
    return (
      <>
        ...
        <Button label="Yes" icon="pi pi-check" className="p-button-text" onClick={() => deleteProduct(product?.idsubtipoproduto as number)} />
      </>
    )
  }
  function actionBodyTemplate(rowData: TypeProductInputs) {
    return (
      <>
        <Button icon="pi pi-pencil" className="p-button-rounded p-button-success mr-2" rounded onClick={() => editProduct(rowData)} />
        <Button icon="pi pi-trash " className="p-button-rounded p-button-warning " onClick={() => confirmDeleteSubTypeProdct(rowData)} />
      </>
    )
  }
  ...
  return (
    <div className='datatable-crud mx-8 mt-5'>
      <div className='cars' >
        ...
        <DataTable value={products} footer={footer} >
          <Column field="idsubtipoproduto" header="Código"></Column>
          <Column field="descricao" header="Name" ></Column>
          <Column body={actionBodyTemplate} exportable={true} className='flex flex-row justify-content-end' header='Actions'></Column>
        </DataTable>
      </div>
      <Dialog
        visible={productDialog}
        header="Sub Tipo Produto"
        modal
        className="absolute"
      ...
      >
        <form onSubmit={handleSubmit(handleCreateOrEditTypeProduct)} >
          ...
          <div className='flex flex-row gap-2 justify-content-end mt-4'>
            ...
            <Button label="No" icon="pi pi-times" className="p-button-text" type='reset' onClick={hideDialog} />
          </div>
        </form>
      </Dialog>
      <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirm" footer={deleteProductDialogFooter} modal onHide={hideDialog}>
